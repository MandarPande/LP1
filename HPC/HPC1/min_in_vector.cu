...
__global__ void minPerBlock(const int *gArr, int arraySize, int *gOut) {
...
int main() {
    ...
    minPerBlock<<<gridSize, blockSize>>>(dev_arr, wholeArraySize, dev_out);
    //dev_out now holds the partial result
    minPerBlock<<<1, blockSize>>>(dev_out, gridSize, dev_out);
